...
module.exports.init = function (server, log_file_position, callback) {
    io = require('socket.io')(server);
    if(typeof callback == 'undefined') {
        callback = function() {};
    }
    log_file_position = log_file_position;
    ...
    */
    ...
    io.on('connection', function (socket) {
        readLogFile(function(statement,last_line_of_text) {
            last_lines = statement.slice(-1000);
...
function watchFile() {
    fs.watchFile(this.log_file_position,{interval:1000}, function (curr, prev) {
        readLogFile(function(statement,last_line_of_text) {
            last_lines = statement.slice(-100);
            io.sockets.emit('log-event', last_lines)
        });
...
function readLogFile(callback) {
    ...
    instream.on('newLine',function(line_of_text, end_of_file){       
        if(!end_of_file){
            ...
            //line_of_text = statement[statement.length-1]
            callback(statement,statement[statement.length-1]);
        }
    });
...
function writeToLog(log_item,callback) {
    ...
    fs.appendFile(log_file_position, log_item+"\n", function (err) {
        if (err) {
            throw err;
        }
